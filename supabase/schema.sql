...
-- Comptes : les soldes des comptes non crédit sont saisis sous forme
-- d'instantanés manuels. Les allocations d'investissement gardent elles
-- aussi un historique par date.
...
create policy "household_scoped_accounts" on accounts
  for all using (household_id = auth_household_id()) with check (household_id = auth_household_id());
create policy "household_scoped_account_balance_snapshots" on account_balance_snapshots
  for all using (household_id = auth_household_id()) with check (household_id = auth_household_id());
create policy "household_scoped_investment_allocations" on investment_allocations
  for all using (household_id = auth_household_id()) with check (household_id = auth_household_id());
