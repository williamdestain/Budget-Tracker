...
describe('BudgetStore (intégration avec faux Supabase)', () => {
  ...
  // Angles morts laissés par les tests "schéma Member" ci-dessus (audit du
  // 14 septembre 2026) : ils couvrent bien les écritures simples table par
  // table, mais pas le chemin le plus délicat de toute la migration 024
  // (splitVersementIntoProvisions() contourne la limite à 2 membres de la
  // RPC en créant la dépense côté client — voir budget-store.service.ts),
  // ni la preuve qu'une vraie agrégation sur 3+ membres fonctionne (et pas
  // seulement sur 1 membre, ce qui masquerait une boucle qui ne boucle pas
  // vraiment).
  describe('Schéma Member — chemins non couverts par les tests ci-dessus', () => {
    function seedThreeMembers(): void {
      fakeClient.seed('members', [
        {
          id: 'member-alex',
          household_id: TEST_HOUSEHOLD_ID,
    ...
    it('répartit un versement sur une provision avec un vrai 3e membre dans le foyer (schéma Member actif)', async () => {
      seedThreeMembers();
      fakeClient.seed('provisions', [
        {
          id: 'p1', name: 'Vacances', amount: 1000, every_n: 12, interval_unit: 'months',
          start_ym: '2026-01', category: 'Vacances', member_id: 'member-alex',
          auto_recalibrate: false, allocation_percent: 0, rolling_count: 0,
        },
      ]);
      await store.loadAll();
      store.activeOwner.set('member-jc'); // reçoit -> l'expéditeur doit être un AUTRE membre réel, pas un 2e profil supposé

      await store.splitVersementIntoProvisions(100, '2026-07-15', [{ provisionId: 'p1', amount: 100 }]);

      // La dépense de versement est créée par le store lui-même (pas par
      // la RPC, qui garde la règle historique à 2 membres) — avec le bon
      // expéditeur/destinataire explicites, pas une déduction implicite.
      const versement = store.expenses().find((e) => e.category === 'Versement');
      ...
      // La RPC a bien rattaché l'ajustement à CETTE dépense créée côté
      // client (p_existing_expense_id), pas à une nouvelle qu'elle aurait
      // créée elle-même.
      const p1 = store.provisions().find((p) => p.id === 'p1');
      expect(p1?.adjustments.some((a) => a.amount === 100)).toBe(true);
      expect(fakeClient.tables['expenses']).toHaveLength(1);
    });

    it("si la RPC échoue après la création côté client de la dépense de versement, celle-ci est retirée — pas de versement orphelin (schéma Member)", async () => {
      seedThreeMembers();
      fakeClient.seed('provisions', [
        {
          id: 'p1', name: 'Vacances', amount: 1000, every_n: 12, interval_unit: 'months',
      ...
      await store.loadAll();
      store.activeOwner.set('member-jc');
      ...
      // Le store a compensé lui-même : la dépense qu'il avait créée avant
      // d'appeler la RPC ne doit pas rester en base malgré l'échec de la RPC.
      expect(fakeClient.tables['expenses']).toEqual([]);
      expect(store.expenses().some((e) => e.category === 'Versement')).toBe(false);
    });

    it("rolloverFor('global'), creditCardBalance('global') et versementsRecus() agrègent réellement sur 3 membres actifs, pas seulement sur 1 (schéma Member)", async () => {
      seedThreeMembers();
      fakeClient.seed('rollovers', [
        { id: 'r1', member_id: 'member-alex', ym: '2026-07', amount: 10 },
        { id: 'r2', member_id: 'member-jc', ym: '2026-07', amount: 20 },
        { id: 'r3', member_id: 'member-sam', ym: '2026-07', amount: 30 },
      ]);
      fakeClient.seed('expenses', [
        // Dettes de carte des 3 membres : doivent toutes compter en vue Global.
        { id: 'e1', amount: 50, category: 'Courses', date: '2026-07-05', member_id: 'member-alex', cc: true },
        { id: 'e2', amount: 20, category: 'Courses', date: '2026-07-05', member_id: 'member-jc', cc: true },
        { id: 'e3', amount: 5, category: 'Courses', date: '2026-07-05', member_id: 'member-sam', cc: true },
        // Versement explicite d'Alex vers Sam : ne doit compter QUE pour Sam,
        // pas pour JC (contrairement à l'ancienne règle implicite "l'autre").
        {
          id: 'e4', amount: 15, category: 'Versement', date: '2026-07-10',
          member_id: 'member-alex', versement_to_member_id: 'member-sam', cc: false,
        },
      ]);
      fakeClient.seed('credit_card_payments', [
        { id: 'cc1', member_id: 'member-jc', amount: 5, date: '2026-07-12', note: '' },
      ]);
      await store.loadAll();

      // 10 + 20 + 30, pas juste 2 des 3 (preuve que memberIds() boucle
      // vraiment sur activeMembers(), pas une addition à 2 termes figée).
      expect(store.rolloverFor('global', '2026-07')).toBe(60);
      // (50 + 20 + 5) chargé - 5 payé par JC = 70.
      expect(store.creditCardBalance('global')).toBe(70);
      // Versement explicite : seul Sam l'a reçu.
      expect(store.versementsRecus('member-sam', '2026-07')).toBe(15);
      expect(store.versementsRecus('member-jc', '2026-07')).toBe(0);
      expect(store.versementsRecus('member-alex', '2026-07')).toBe(0);
    });
  });
